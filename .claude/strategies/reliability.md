...
- Use `-DforkCount=1` for Maven Surefire or `maxParallelForks = 1` for Gradle when resource constrained TODO: update to use the tools/quality app given it will generate reports. ### Triaging Failed Tests - Whenever there are failing tests we want to resolve those failures by addressing the root cause causing the failure
- We don't just want to quiet the testing system and thereby swallow errors, failures, or warnings
- use the testing output AND the logs in the logs/ directory, e.g., application.log or test-output.log.
